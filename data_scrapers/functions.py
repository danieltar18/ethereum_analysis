...
def normalized_index(data, lookback, close, position):
    ...
    for i in range(len(data)):
        ...
        try:
            ...
            data[i, position] = (data[i, close] - min(data[i - lookback + 1:i + 1, close])) / (
                        max(data[i - lookback + 1:i + 1, close]) - min(data[i - lookback + 1:i + 1, close]))
...
def heikin_ashi(data, open_column, high_column, low_column, close_column, position):
    ...
    # Heikin-Ashi Close
    for i in range(len(data)):
        data[i, position + 3] = (data[i, open_column] + data[i, high_column] + data[i, low_column] + data[
            i, close_column]) / 4
